  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-16">
      ...
      <div className="max-w-6xl mx-auto px-4 pt-8 space-y-8">
        ...
        {winner && (
          <section className="bg-gradient-to-r from-indigo-600 to-indigo-700 rounded-2xl p-6 text-white">
            <p className="text-indigo-200 text-xs font-semibold uppercase tracking-wide mb-1">
              Winner · {result.urls.length} pages analyzed
            </p>
            <div className="flex items-center gap-4 mt-2">
              <div className="flex-shrink-0">
                ...
                <p className="text-xs text-indigo-200 text-center mt-1">/ 100</p>
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {ranked.map(({ result: r, overall, rank }, idx) => (
                  <tr key={r.url} className={rank === 1 ? "bg-indigo-50/50" : ""}>
                    ...
                    <td className="py-3">
                      <p className="font-medium text-gray-900 truncate max-w-[180px]">{hostname(r.url)}</p>
        ...
        {/* Expanded expert cards */}
        {ranked.map(({ result: r, rank }, idx) =>
          expandedIndex === idx ? (
            <section key={r.url} className="bg-white rounded-2xl border border-indigo-100 shadow-sm p-6">
              <div className="flex items-center gap-3 mb-6">
                <span
                  className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-sm font-bold ${
                    rank === 1 ? "bg-indigo-600 text-white" : "bg-gray-200 text-gray-700"
                  }`}
                >
                  {rank}
                </span>
        ...
        {result.slug && (
          <div className="text-center pb-4">
            ...
            >
              ...
              {copied ? "Link copied!" : "Share this bulk report"}
            </button>
          </div>
        )}
      </div>
