...
export default function ComparePage() {
  ...
  const EXPERT_META: Record<ExpertName, { label: string; icon: string }> = {
    "ui-design": { label: tMeta("uiDesign"), icon: "🎨" },
    "ux-research": { label: tMeta("uxResearch"), icon: "🔍" },
    experiment: { label: tMeta("experiment"), icon: "🧪" },
    content: { label: tMeta("content"), icon: "✍️" },
    seo: { label: tMeta("seo"), icon: "📈" },
    psychology: { label: tMeta("psychology"), icon: "🧠" },
  };
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-gray-400 text-left">
                  <th className="pb-3 font-medium">{t("tableHeaders.expert")}</th>
                  <th className="pb-3 font-medium text-center">
                    {t("tableHeaders.myPage")}
                    <span className="block text-gray-300 font-normal truncate max-w-[120px]">
                      {hostname(result.url1)}
                    </span>
                  </th>
                  <th className="pb-3 font-medium text-center">
                    {t("tableHeaders.competitor")}
                    <span className="block text-gray-300 font-normal truncate max-w-[120px]">
                      {hostname(result.url2)}
                    </span>
                  </th>
                  <th className="pb-3 font-medium text-center">{t("tableHeaders.delta")}</th>
                </tr>
              </thead>
              ...
              return (
                <div key={k}>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <p className="text-xs text-gray-400 mb-2 truncate">
                        {t("competitor")} · {hostname(result.url2)}
                      </p>
                      <ExpertCard
                        name={EXPERT_META[k].label}
                        icon={EXPERT_META[k].icon}
